...
var prerenderURL = process.env.PRERENDER_URL;
...
module.exports = {
    beforePhantomRequest: function (req, res, next) {
        // 
        if (req.method === 'POST' && redis_online === true && req.url === "/recache") {
            let rawData = '';
            req.on('data', (chunk) => { rawData += chunk; });
            req.on('end', () => {
                try {
                    const parsedData = JSON.parse(rawData);
                    if (parsedData.prerenderToken === process.env.TOKEN) {
                        client.del(parsedData.url, function (err, result) {
                            http.get(prerenderURL+"/"+parsedData.url);
                            return res.send(200, 'recached');
                        });
                    } else {
